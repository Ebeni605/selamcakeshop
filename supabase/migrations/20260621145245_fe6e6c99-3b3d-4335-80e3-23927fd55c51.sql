...
BEGIN
  ...
  END IF;
  trunc_unit := CASE lower(_granularity) WHEN 'week' THEN 'week' WHEN 'month' THEN 'month' ELSE 'day' END;
  RETURN QUERY
  SELECT
    date_trunc(trunc_unit, o.created_at) AS bucket,
    COUNT(*)::BIGINT AS order_count,
    COALESCE(SUM((SELECT COALESCE(SUM((i->>'quantity')::INT),0) FROM jsonb_array_elements(o.items) AS i)),0)::BIGINT AS units_sold,
    COALESCE(SUM(o.total),0)::NUMERIC AS revenue
  FROM public.orders o
  WHERE o.created_at >= _from AND o.created_at < _to
  GROUP BY 1 ORDER BY 1;
END; $$;
REVOKE EXECUTE ON FUNCTION public.get_sales_analytics(TEXT, TIMESTAMPTZ, TIMESTAMPTZ) FROM PUBLIC, anon;
